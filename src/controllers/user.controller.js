...
export const getUserChannelProfile = asyncHandler(async (req, res, next) => {
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $addFields: {
                subscribersCount: {
                    $size: "$subscribers"//because a fields
                },
    ...
    //will contain only 1 in our case
    if(!channel?.length)
...
export const getWatchHistory = asyncHandler(async (req, res, next) => {
        ...
        },
        {
            $lookup: {
                ...
                pipeline: [
                    ...
                    {
                        //data in owner field array of objects need only first element
                        $addFields: {
                            owner: {
                                $first: "$owner"
                            }
    ...
    return res
           .status(200)
           .json(
            new ApiResponse(200, user[0].wacthHistory, "watch history fetched successfully")
           )
})
